...
if [ ! -f "$ENV_FILE" ]; then
    ...
    printf '      1 - Anthropic / Claude (рекомендуется, нужен API-ключ)\n'
    printf '      2 - OpenAI (нужен API-ключ)\n'
    printf '      3 - Ollama (локальные модели, без ключа; Ollama должна быть установлена)\n'
    printf '      Enter - пропустить, настроить позже\n\n'
    read -r -p "    Выберите провайдера (1/2/3 или Enter): " choice || choice=""
    ...
    case "$choice" in
        ...
        2)
            ...
            if [ -n "${key// /}" ]; then
                ...
                ENV_LINES+=("CAMPAIGN_LLM_MODEL_COMPOSITION=gpt-4o")
                HAS_OPENAI_KEY=1
            else
                warn "Ключ пустой - пропускаю настройку."
            fi
            ;;
        3)
            read -r -p "    Имя модели Ollama (Enter = llama3.1; модель должна быть скачана: ollama pull <имя>): " model || model=""
            model="${model// /}"
            [ -n "$model" ] || model="llama3.1"
            ...
            ENV_LINES+=("CAMPAIGN_LLM_MODEL_COMPOSITION=$model")
            ;;
    esac
    ...
    if [ "${#ENV_LINES[@]}" -gt 0 ]; then
        ...
        printf '      2 - OpenAI (качественнее, но текст лора уходит в OpenAI; нужен OpenAI-ключ)\n'
        printf '      3 - Отключить (ассистент будет хуже находить связанный лор)\n\n'
        read -r -p "    Выберите (1/2/3 или Enter = 1): " emb_choice || emb_choice=""
        case "$emb_choice" in
            2)
